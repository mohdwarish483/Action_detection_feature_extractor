...
class I3Res50(nn.Module):
    ...
    def __init__(
        self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=400, use_nl=False,classify=False
    ):
        self.inplanes = 64
    ...
    def forward_single(self, x):
        print("Calling shape in resnet 50 forward single", x.shape)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool1(x)
        ...
        x = self.layer4(x)
        x = self.avgpool(x)
        print("Calling shape after avg and fc layer", x.shape)
        if self.classify:
            #reshape the output with view by number of smaples and - 1 is to automatically calculate the dimension size based on remaining dimension --> 2048,400x1x1 = 2048,400
            x = x.view(x.size(0),-1)
            print("===========Model Classification ===================")
            x=self.fc(x)
            print("Final classes features shape at fc layer",x.shape)
        return x
    ...
    def forward(self, batch):
        ...
        print(
            "calling the shape of the one batch features output in model forward method",feat.shape,)
        return feat
...
def i3_res50(num_classes,pretrainedpath,classify):
    net = I3Res50(num_classes=num_classes, use_nl=False,classify=classify)
